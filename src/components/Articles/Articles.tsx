...
export const Articles: React.FC = () => {
  ...
  const [articles, setArticles] = React.useState<ArticlesAPI>({ items: [], categories: [], sources: [] });

  React.useEffect(() => {
    fetch('https://frontend.karpovcourses.net/api/v2/ru/news/' + categoryIds[categoryId] || '0')
      .then((response) => response.json())
      .then((response: ArticlesAPI) => {
